...
from airflow.utils import dag_backup_helper
...
def backup_dags(args):
    import tarfile
    airflow_home = os.environ['AIRFLOW_HOME']
    dags_folder = airflow_home + '/dags'
    logging.info("Taking dags backup...")
    if os.path.exists(dags_folder):
        #Add this block to try catch, figure out the out files location
        tar = tarfile.open('dags.tar.gz', mode = 'w:gz')
        for f in os.listdir(dags_folder):
            tar.add(dags_folder + '/' + f, arcname=f)
        tar.close()

        if dag_backup_helper.backup_folder_exists():
            # if args.force is True:
            #     push_to_s3(dags_folder + '/dags.tar.gz')
            # else:
            question = (
                'Seems like you already have taken a backup in the past, executing this '
                'command will overwrite that backup. '
                'Are you sure you want to proceed? (y/n)')

            if not utils.helpers.ask_yesno(question):
                logging.info('Bailing out, backup not taken')
            else:
                dag_backup_helper.push_to_s3(dags_folder + '/dags.tar.gz')
                logging.info("Backup Successfully taken")
        else:
            dag_backup_helper.push_to_s3(dags_folder + '/dags.tar.gz')
            logging.info("Backup Successfully taken")

    else:
        raise AirflowException('Dags folder not found. Please ensure that all your dags are stored in $AIRFLOW_HOME/dags')

def restore_dags(args):
    import tempfile
    logging.info("Fetching backup...")
    remote_base_path = conf.get('core', 'REMOTE_BASE_LOG_FOLDER')
    remote_loc = remote_base_path + '/dags_backup/dags.tar.gz'
    tempDir = tempfile.mkdtemp()
    dag_backup_helper.pull_from_s3(remote_loc, tempDir)
    dag_backup_helper.untar_and_save_dags(tempDir)
    logging.info("Backup dags successfully fetched and stored")
...
class CLIFactory:
    ...
    subparsers = [
        ...
        # In this version all above entries have been given a name, may we need to give name to below entries as well
        {
            'func': backup_dags,
            'help': "Takes the backup of the dags by pushing its tar file to s3",
            'args': '',
        }, {
            'func': restore_dags,
            'help': "Gets the backup dags from s3",
            'args': '',
        },
    ]
