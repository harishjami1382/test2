...
from airflow.utils.qubole_helper import read_from_root_config_json
...
def create_default_connections(session=None):
    ...
    """
    google_project_id = read_from_root_config_json(["cloud_config", "project_id"])
    merge_conn(
        Connection(
            conn_id="airflow_db",
            conn_type="mysql",
    ...
    merge_conn(
        Connection(
            ...
            schema="default",
            extra='{{ "extra__google_cloud_platform__project": "{}" }}'.format(google_project_id)
        ),
        session
    )
    merge_conn(
    ...
    merge_conn(
        Connection(
            ...
            conn_type="qubole",
            host=os.getenv('QUBOLE_BASE_URL') + '/api'
        ),
        session
    )
    merge_conn(
