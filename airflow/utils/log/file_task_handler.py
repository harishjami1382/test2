...
class FileTaskHandler(logging.Handler):
    ...
    def close(self):
        """
        close file handler and perform clean up steps
        """
        if self.handler:
            self.handler.close()
    ...
    def _read(self, ti, try_number, metadata=None) -> (str, Dict[str, bool]):  # pylint: disable=unused-argument
        """
        ...
        if os.path.exists(location):
            try:
                ...
                log, error = "*** Failed to load local log file: {}\n".format(location), True
                log += "*** {}\n".format(str(e))
        else:
            url = os.path.join(
        ...
        return log, {'end_of_log': True, 'error_while_fetch': error}
