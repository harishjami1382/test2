...
def send_mime_email(e_from, e_to, mime_msg, dryrun=False):
    ...
    request_timeout = conf.get('smtp', 'CONNECTION_TIMEOUT')
    smtp_user = None
    smtp_password = None
    ...
    if not dryrun:
        conn = smtplib.SMTP_SSL(host=smtp_host, port=smtp_port, timeout=request_timeout) if smtp_ssl else \
            smtplib.SMTP(host=smtp_host, port=smtp_port, timeout=request_timeout)
        if smtp_starttls:
            conn.starttls()
