...
from flask import (
    ...
    send_file, session as flask_session, url_for, g
)
from flask_appbuilder import BaseView, ModelView, expose, permission_name
from airflow.utils.qubole_utils import has_access
...
from airflow.utils.qubole_utils import proxified_get_home, clean_url
...
def dagbag():
    if 'dagbag' not in g:
        if os.environ.get('SKIP_DAGS_PARSING') != 'True':
            g.dagbag = models.DagBag(settings.DAGS_FOLDER)
        else:
            g.dagbag = models.DagBag
    return g.dagbag
...
class Airflow(AirflowBaseView):
    ...
    def home(self, session=None):
        return redirect(proxified_get_home())
    ...
    def redirect(self):
        dag_id = request.args.get('dag_id')
        task_id = request.args.get('task_id')
        execution_date = dateutil.parser.parse(request.args.get('execution_date'))
    ...
    def run(self):
        ...
        origin = clean_url(request.form.get('origin'), True)
        dag = dagbag().get_dag(dag_id)
    ...
    def delete(self):
        ...
        origin = clean_url(request.form.get('origin'), True) or proxified_get_home()
    ...
    def trigger(self, session=None):
        ...
        dag_id = request.values.get('dag_id')
        origin = clean_url(request.values.get('origin'), True) or proxified_get_home()
    ...
    def gantt(self, session=None):
        ...
        for gantt_bar_item in gantt_bar_items:
            ...
            start_date = gantt_bar_item[1] if gantt_bar_item[1] else dttm
...
class PoolModelView(AirflowModelView):
    ...
    def pool_link(attr):
        ...
        if pool_id is not None:
            url = \
                url_for('TaskInstanceModelView.list', _flt_3_pool=pool_id)
