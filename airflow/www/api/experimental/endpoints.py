...
@api_experimental.route('/sync/<string:sync_type>', methods=['GET'])
@requires_authentication
def sync_from_remote(sync_type):
    """Sync dags/plugins from remote"""
    import subprocess
    import os
    from airflow.models import DagBag
    from airflow import settings

    accepted_types = ["dags", "plugins"]
    if sync_type not in accepted_types:
        return jsonify(error="only dags and plugins sync is supported, {0} is invalid type".format(sync_type))

    airflow_home = os.getenv("AIRFLOW_HOME", "/usr/lib/airflow")
    cmd = ["/bin/bash",
           "{0}/airflow/qubole_assembly/setup_crons.sh".format(airflow_home),
           "airflow_sync_{0}_from_remote".format(sync_type)]
    command = subprocess.Popen(cmd)
    command.wait()

    # update dagbag for web server
    g.dagbag = DagBag(settings.DAGS_FOLDER)

    # update dags for scheduler
    file_to_check = str(os.getenv("AIRFLOW_HOME", "/usr/lib/airflow")) + "/scheduler.refresh"
    if not os.path.exists(file_to_check):
        with open(file_to_check, 'w'):
            pass

    return jsonify("{0} sync from remote initiated".format(sync_type))

@api_experimental.route('/update_sync_location', methods=['GET'])
@requires_authentication
def update_sync_location():
    """Update Sync Location from Qubole using API Call"""
    import subprocess
    import os
    ...
    return jsonify("{0} sync location updated")


@csrf.exempt
@api_experimental.route('/delete_dag_with_name/<string:name>', methods=['DELETE'])
@requires_authentication
def delete_dag_with_name(name):
    """Delete dag from cluster and remote location"""
    import subprocess
    import os

    airflow_home = os.getenv("AIRFLOW_HOME", "/usr/lib/airflow")
    cmd = ["/bin/bash",
           "{0}/airflow/qubole_assembly/scripts/delete_dag_with_name.sh".format(airflow_home),
           name]
    ...
    return jsonify({'success':"DAG : {0} deleted successfully from the cluster and remote location, To delete"
                              " DAG metadata, Delete it from the Webserver UI.".format(name)})
