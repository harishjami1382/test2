...
class DAG(BaseDag, LoggingMixin):
    ...
    def delete(self, session=None):
        ...
        if len(active_runs) > 0:
            raise AirflowException("There are active runs of this DAG as of now, can't delete the DAG.")

        if self.full_filepath is not None:
            try:
                if os.path.isfile(self.full_filepath):
                    os.remove(self.full_filepath)
                if os.path.isfile(self.full_filepath + "c"):
                    os.remove(self.full_filepath + "c")

                print("Deleted the DAG file at location {}. Now deleting DB entries.".format(self.full_filepath))
            except:
                raise AirflowException("Couldn't remove the DAG file at location {}. Bailing out.".format(self.full_filepath))
        else:
            print("DAG full filepath couldn't be found a. Moving ahead to delete DB entries.")
        ...
        session.query(TaskInstance).filter(TaskInstance.dag_id == self.dag_id).delete()
        ...
        session.query(DagStat).filter(DagStat.dag_id == self.dag_id).delete()
        session.commit()
        print("Successfully deleted DAG {}.".format(self.dag_id))
